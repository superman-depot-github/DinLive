package com.dinlive.din.baselib.wiget.stateview;
...
public class StateView extends View {
    ...
    private int emptyDrawableResId = 0;
     ...
     * 设置空数据视图 显示图片
     *
     * @param emptyDrawableResId 图片资源ID
     * @return
     */
    public StateView setEmptyDrawableResId(int emptyDrawableResId) {
     ...
     * 设置空数据视图 显示标题
     *
     * @param emptyTitle 标题文本
     * @return
     */
    public StateView setEmptyTitle(String emptyTitle) {
     ...
     * 设置空数据视图 重试按钮文本
     *
     * @param emptyBtnText 按钮文本
     * @return
     */
    public StateView setEmptyBtnText(String emptyBtnText) {
     ...
     * 设置重试加载视图 显示图片
     *
     * @param retryDrawableResId 图片资源ID
     * @return
     */
    public StateView setRetryDrawableResId(int retryDrawableResId) {
        this.retryDrawableResId = retryDrawableResId;
        return this;
    }
     ...
     * 设置重试加载视图 标题文本
     *
     * @param retryTitle 按钮文本
     * @return
     */
    public StateView setRetryTitle(String retryTitle) {
     ...
     * 设置重试加载视图 重试按钮文本
     *
     * @param retryBtnText 按钮文本
     * @return
     */
    public StateView setRetryBtnText(String retryBtnText) {
    ...
    public View showEmpty() {
        if (mEmptyView == null) {
            ...
            btnRetry.setVisibility(!TextUtils.isEmpty(emptyBtnText) ? VISIBLE : GONE);
            if (!TextUtils.isEmpty(emptyBtnText) && btnRetry.getVisibility() == VISIBLE) {
                btnRetry.setText(emptyBtnText);
                btnRetry.setOnClickListener(v -> {
                    if (mOnEmptyClickListener != null) {
                        //showLoading();
                        btnRetry.postDelayed(() -> mOnEmptyClickListener.onEmptyClick(), 300);
                    }
                });
            }
        }
    ...
    public View showRetry() {
        if (mRetryView == null) {
            ...
            if (!TextUtils.isEmpty(retryBtnText)) {
                btnRetry.setText(retryBtnText);
            }
            btnRetry.setOnClickListener(v -> {
                if (mRetryClickListener != null) {
                    //showLoading();
                    btnRetry.postDelayed(() -> mRetryClickListener.onRetryClick(), 300);
                }
            });
        }
    ...
    public View showLoading() {
        if (mLoadingView == null) {
            ...
            //ImageView progressWheel = mLoadingView.findViewById(R.id.progress_wheel);
            //Glide.with(getContext()).asGif().load(R.drawable.state_view_loading).into(progressWheel);
        }
     ...
     */
    public StateView setOnRetryClickListener(OnRetryClickListener listener) {
     ...
     * 空布局的监听重试
     *
     * @param listener {@link OnEmptyClickListener}
     */
    public StateView setOnEmptyClickListener(OnEmptyClickListener listener) {
        this.mOnEmptyClickListener = listener;
        return this;
    }

    /**
     * Listener used to receive a notification after the EmptyView is clicked.
     */
